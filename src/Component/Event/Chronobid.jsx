...
export default function ChronoBid() {
  ...
  const event = {
    ...
    contact: ["+91 94449 62904", "+91 90257 70052"],
  };

  const checkTeamLimit = async () => {
    try {
      const res = await axios.get(
        import.meta.env.VITE_BACKEND_URL + "/chrono_bid/count"
      );
  ...
  const textFlicker = {
    animate: {
      textShadow: [
        "0 0 5px #fff, 0 0 5px #fff, 0 0 15px #0ff, 0 0 20px #0ff, 0 0 25px #0ff",
        "0 0 2px #fff, 0 0 5px #fff, 0 0 7px #0ff, 0 0 10px #0ff, 0 0 15px #0ff",
        "0 0 5px #fff, 0 0 10px #fff, 0 0 15px #0ff, 0 0 20px #0ff, 0 0 25px #0ff",
      ],
      transition: {
        duration: 1.5,
        repeat: Infinity,
