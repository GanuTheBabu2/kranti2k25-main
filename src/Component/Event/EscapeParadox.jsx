...
import escapeimage from "../Event/images/escape.webp";

export default function EscapeParadox() {
  const [isOpen, setIsOpen] = useState(false);
  const [teamLimitReached, setTeamLimitReached] = useState(false);
  const [formData, setFormData] = useState({
    teamName: "",
    event: "Escape Paradox",
    member1: {
      name: "",
      phone: "",
      email: "",
  ...
  const event = {
    ...
    rounds: [
      ...
      },
      {
        ...
        details:
          "Non-coding members find an object in a given set of pictures using a clue-based map. The last two teams face a surprise coding challenge.",
      },
      {
        title: "Winner Announcement",
        details:
          "Judged on creativity, technical skill, teamwork, and time management.",
      },
    ],
    rules: [
      "Teams of 2 members.",
      "Complete all challenges within the time limit.",
      "External help leads to disqualification.",
    ...
    contact: ["+91 63835 37126", "+91 90257 70052"],
  };

  const checkTeamLimit = async () => {
    try {
      const res = await axios.get(
        import.meta.env.VITE_BACKEND_URL + "/escape_paradox/count"
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        import.meta.env.VITE_BACKEND_URL + "/escape_paradox",
        formData
      );
      if (res.status === 200) {
        ...
        toast.success("Team name already exist");
      } else {
